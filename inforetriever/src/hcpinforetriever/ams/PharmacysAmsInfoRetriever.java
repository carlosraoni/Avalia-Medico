...
import hcpinforetriever.parser.ams.PharmacysAmsHtmlParser;
...
public class PharmacysAmsInfoRetriever {
	...
	private void retrieveAndSaveToJSONAmsInfo(List<BrazilState> states, String directoryPath) {
		...
		try{			
			System.out.println("#################### Obtendo informações das fármacias da AMS ####################");			
			...
			for(BrazilState state: states){
				...
				PharmacysAmsOnlineInformationExtractor extractor = new PharmacysAmsOnlineInformationExtractor(state.toString());								
				...
				PharmacysAmsHtmlParser parser = new PharmacysAmsHtmlParser(state.toString(), html);
				List<Pharmacy> pharmacys = parser.parse();
				
				System.out.println("Gerando arquivo JSON com as informações ...");
				PharmacysJSONFormatter JSONFormatter = new PharmacysJSONFormatter();							
				String json = JSONFormatter.format(pharmacys);				
				...
				System.out.println("Informações das fármacias da AMS do estado do " + state +" salvas com sucesso no arquivo: "+ filePath);
	...
	public static void main(String[] args) {
		...
		try {
			...
			PharmacysAmsInfoRetriever amsInfoRetriever = new PharmacysAmsInfoRetriever();
